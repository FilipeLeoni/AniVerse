...
const Sidebar = () => {
  ...
  const tabContainerRef: any = React.useRef<HTMLElement>(null);

  const handleTabSelect = () => {
    tabContainerRef.current.scrollTop = 0;
  };

  const ref: any = useHorizontalScroll<HTMLDivElement>();
    ...
    >
      <div className="p-2 bg-background-800">
        <TabList className="z-50 sticky top-0">
          ...
          >
            <Tab className="px-3 py-2 cursor-pointer">Introduction</Tab>
            <Tab className="px-3 py-2 cursor-pointer">Text chat</Tab>
      ...
      <div className="py-4 grow overflow-x-hidden no-scrollbar">
        <TabPanel className="hidden p-2">{/* <MediaBar /> */}</TabPanel>
        <TabPanel className="hidden h-full p-2">
          <ChatBar />
        </TabPanel>
        <TabPanel className="hidden h-full p-2">{/* <AudioChat /> */}</TabPanel>
      </div>
    </Tabs>
  );
};
